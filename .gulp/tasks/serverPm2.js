...
module.exports = (config) => {
  ...
  gulp.task('pm2-start', () => {
    exec('pm2 start server -n ' + app_name, (err, stdout) {
      if (err) {
        console.log(err);
      } else {
  ...
  gulp.task('pm2-stop', () {
    exec('pm2 stop server --silent -n ' + app_name, (err, stdout) => {
      if (err) {
        console.log(err);
      } else {
  ...
  gulp.task('pm2-restart', () => {
    exec('pm2 restart server -n ' + app_name, (err, stdout) {
      if (err) {
        console.log(err);
      } else {
  ...
  gulp.task('pm2-delete', () {
    exec('pm2 delete server -n ' + app_name, (err, stdout) => {
      if (err) {
        console.log(err);
      } else {
  ...
  gulp.task('pm2-deleteall', () {
    exec('pm2 delete all', (err, stdout) => {
      if (err) {
        console.log(err);
      } else {
